"""
Phase 0 Comprehensive Evaluation Script
----------------------------------------
Executes locked, fully reproducible evaluation across:
1. Q1 Clean Baseline
2. Q2 Non-Adaptive Attacks (FGSM, 30-step PGD-T1)
3. Q3 Adaptive Attack T2 Lambda-Sweep (lambda in {0.0, 0.1, 1.0, 10.0, 50.0, 100.0}, 30 steps)

Integrates:
- Degeneracy spatial variance thresholding (sigma < 1e-5, dying-ReLU detection)
- Selection bias cross-tabulation (Degeneracy x Label-Flip Success)
- Unfiltered, Filtered, Flipped-Only, and Flipped+Filtered Cosine & JS AUCs
- 95% Confidence Intervals via bootstrapping (B=1000)
- Fixed dataset split (150 real, 150 AI) and seed (42)
"""
...
def run_comprehensive_eval():
    ...
    joint_model = JointEnsembleModel(art_model, sem_model).to(device).eval()
    ...
    N = len(images)
    print(f"Dataset locked: Total={N} (Real=150, AI=150)")
    ...
    print(f"Clean Degeneracy: ResNet={clean_deg_a/N*100:.1f}%, CLIP={clean_deg_b/N*100:.1f}%")
    
    # Define attack suite
    # Q2: FGSM and PGD-T1 (30 steps, eps=8/255, alpha=2/255)
    # Q3: Adaptive T2 Lambda Sweep (30 steps, eps=8/255, alpha=2/255)
    ...
    full_results = {}
    y_true_all = np.concatenate([np.zeros(N), np.ones(N)]) # 0=Clean, 1=Adversarial
    ...
    for name, mode, lam in configs:
        ...
        # Analyze metrics across N samples
        cos_adv, js_adv = [], []
        deg_a_count, deg_b_count = 0, 0
        is_deg_flags = []
        is_flipped_flags = []
        
        for idx in tqdm(range(N), desc=f"Grad-CAM {name}"):
        ...
        # 2x2 Cross Tabulation: Degeneracy x Label-Flip
        ...
        # 1. Unfiltered AUC (All N clean vs All N adv)
        ...
        # 2. Filtered AUC (Valid clean vs Valid adv)
        valid_mask_adv = ~is_deg_flags
        valid_cos_adv = cos_adv[valid_mask_adv]
        valid_clean_cos = clean_cos # 300 clean samples
        ...
        # 3. Flipped-Only Unfiltered AUC (Clean vs Flipped Adv)
        if n_flipped > 0:
            flipped_cos_adv = cos_adv[is_flipped_flags]
            y_true_flip = np.concatenate([np.zeros(len(clean_cos)), np.ones(n_flipped)])
            scores_flip = np.concatenate([clean_cos, flipped_cos_adv])
            auc_flip_unfilt = float(roc_auc_score(y_true_flip, scores_flip))
            ci_flip_unfilt = bootstrap_auc_ci(y_true_flip, scores_flip)
        else:
            auc_flip_unfilt = float('nan')
            ci_flip_unfilt = (float('nan'), float('nan'))
            
        # 4. Flipped + Filtered AUC (Clean vs Valid & Flipped Adv)
        ...
        full_results[name] = {
            ...
            'auc_flipped_unfiltered': round(auc_flip_unfilt, 4) if not np.isnan(auc_flip_unfilt) else None,
            'ci_flipped_unfiltered': [round(ci_flip_unfilt[0], 4), round(ci_flip_unfilt[1], 4)] if not np.isnan(ci_flip_unfilt[0]) else None,
    ...
    with open('./results/phase0_comprehensive_summary.json', 'w') as f:
